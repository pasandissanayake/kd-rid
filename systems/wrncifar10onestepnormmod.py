...
class StudentModelRed(SystemModel):
    ...
    def get_loss_fn(self, batch, batch_idx, expand=None):
        ...
        def loss_fn(y, labels):
            ...
            distill_loss = ce_1 + torch.mean(torch.log(hsvar1)) / ce_1 \
                            + torch.mean(torch.square(y[2][0]-y[3][0]) / hsvar1) \
                         + ce_2 + torch.mean(torch.log(hsvar2)) / ce_2 \
                            + torch.mean(torch.square(y[2][1]-y[3][1]) / hsvar2) \
                         + ce_3 + torch.mean(torch.log(hsvar3)) / ce_3 \
                            + torch.mean(torch.square(y[2][2]-y[3][2]) / hsvar3)

            soft_loss = 0
            if self.late_kd_epoch is not None and self.current_epoch < self.late_kd_epoch:
                distill_loss = 0
            else:
                distill_loss = self.alpha[1] * distill_loss
            
            if expand:
                self.labels.extend(labels.cpu().numpy())
                return student_loss * self.alpha[0] + distill_loss, student_loss, mse_loss, log_loss, soft_loss
            else:
                self.labels.extend(labels.cpu().numpy())
                return student_loss * self.alpha[0] + distill_loss
