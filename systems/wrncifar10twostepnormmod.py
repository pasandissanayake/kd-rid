...
class StudentModelRed(SystemModel):
    def __init__(self, temperature, lr, alpha, teacher, teacher_filters, data_size_factor, late_kd_epoch):
        super().__init__(num_classes=10)
    ...
    def get_loss_fn(self, batch, batch_idx, expand=None):
        ...
        def loss_fn(y, labels):
            # predictions, softlabels, [f(S)'s], [f(T)'s]
            ...
            ce_1 = F.cross_entropy(y[4][0], labels)
            ce_2 = F.cross_entropy(y[4][1], labels)
            ce_3 = F.cross_entropy(y[4][2], labels)

            distill_loss = torch.mean(torch.log(hsvar1) + torch.square(y[2][0]-y[3][0]) / hsvar1) / ce_1 \
                         + torch.mean(torch.log(hsvar2) + torch.square(y[2][1]-y[3][1]) / hsvar2) / ce_2 \
                         + torch.mean(torch.log(hsvar3) + torch.square(y[2][2]-y[3][2]) / hsvar3) / ce_3
            
            mse_loss = ce_1
            log_loss = distill_loss
            soft_loss = torch.norm(hsvar1)
            ...
            if expand:
                ...
                return student_loss * self.alpha[0] + self.alpha[1] * distill_loss, student_loss, mse_loss, log_loss, soft_loss
            else:
                self.labels.extend(labels.cpu().numpy())
                return student_loss * self.alpha[0] + self.alpha[1] * distill_loss
...
class System():
    ...
    def init_student_red(self, lr, alpha, data_size_factor, late_kd_epoch=None):
        ...
        self.student_red = StudentModelRed(self.temp, lr, alpha, self.teacher, self.filters_red, data_size_factor, late_kd_epoch)
